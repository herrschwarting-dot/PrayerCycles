 ...
 */
export function TagInput({ tags, onChange, placeholder, allTags = [] }: TagInputProps) {
  ...
  return (
    <div className="relative">
      <div
        className="flex flex-wrap items-center gap-1.5 rounded-lg bg-slate-700 px-3 py-2 min-h-[38px] cursor-text"
        onClick={() => inputRef.current?.focus()}
      >
        {tags.map((tag) => (
          <span
            key={tag}
            className="flex items-center gap-1 rounded-full bg-slate-600 px-2 py-0.5 text-xs text-slate-200"
          >
        ...
        <div className="flex items-center min-w-[80px] flex-1">
          {showHash && <span className="text-sm text-slate-400 select-none">#</span>}
          <input
            ref={inputRef}
            type="text"
            value={input}
      ...
      {suggestions.length > 0 && (
        <div className="absolute left-0 right-0 top-full z-10 mt-1 rounded-lg bg-slate-700 py-1 shadow-lg border border-slate-600">
          {suggestions.map((tag) => (
            <button
              key={tag}
              type="button"
              ...
              className="w-full px-3 py-1.5 text-left text-sm text-slate-200 hover:bg-slate-600"
            >
